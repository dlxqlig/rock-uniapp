<template>
	<view class="content">
		...
		<view class="row b-b">
			...
			<input placeholder="请选择城市" disabled="true" :value="addressData.province + ' ' + addressData.city" @click="lotusAddressData.visible = true" class="input">
	...
	export default {
		...
		methods: {
			...
			confirm(){
				...
				if(!data.name){
					this.$api.api.toast('请填写收货人姓名');
					return;
				}
				if(!/(^1[3|4|5|7|8][0-9]{9}$)/.test(data.phoneNumber)){
					that.$api.api.toast('请输入正确的手机号码');
					return
				}
				if (!data.province) {
					that.$api.api.toast('请选择省市区');
					return
				}
				if (!data.city) {
					that.$api.api.toast('请选择二级城市')
					return
				}
				if(!data.detailAddress){
					that.$api.api.toast('请输入详细地址');
					return
				}
				
				if (that.manageType === 'edit') {
					that.$api.api.request('/member/address/update', 'POST', {
						...that.addressData,
						id : that.addressData.id
					}).then(res => {
						that.$api.api.prePage().refreshList(data, that.manageType);
						uni.navigateBack()
					})
				} else {
					that.$api.api.request('/member/address/add', 'POST', that.addressData).then(res => {
						that.$api.api.prePage().refreshList(data, that.manageType);
						uni.navigateBack()
					})
				}
