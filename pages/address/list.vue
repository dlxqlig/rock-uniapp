		...
		<view class="list b-b" v-for="(item, index) in addressList" :key="index" @click="checkAddress(item)" @longpress="deleteShow(item)">
			<view class="wrapper">
				...
				<view class="address-box">
					...
					<text class="address">{{item.province}} {{item.city}} {{item.detailAddress}}</text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			checkAddress(item) {
				if (this.source == 1) {
					this.$api.api.prePage().defaultAddress = item
					uni.navigateBack()
				}
			},
			...
			refreshList(data, type) {
				const that = this
				that.$api.api.request('/member/address/list', 'GET', {}).then(res => {
			...
			deleteShow(item) {
				...
				uni.showModal({
					...
					success: (e) => {
						if (e.confirm) {
							that.$api.api.request('/member/address/delete', 'POST',
							{id:item.id}).then(res => {
								that.$api.api.toast("删除成功！")
							})
						}
					},
					fail: () => {}
