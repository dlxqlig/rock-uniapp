					...
					<view v-for="(item,index) in tabItem.orderList" :key="index" class="order-item">
						<navigator :url="'/pages/order/detail?orderid=' + item.id">
							...
							<scroll-view v-if="item.orderItemList.length > 1" class="goods-box" scroll-x>
								<view v-for="(orderItem, orderIndex) in item.orderItemList" :key="orderIndex" class="goods-item">
									<image class="goods-img" :src="orderItem.productPic" mode="aspectFill"></image>
								</view>
							</scroll-view>
							<view v-if="item.orderItem.length === 1" class="goods-box-single" v-for="(orderItem, orderIndex) in item.orderItem" :key="orderIndex">
								<image class="goods-img" :src="orderItem.productPic" mode="aspectFill"></image>
								<view class="right">
									<text class="attr-box">{{orderItem.productName}} x {{orderItem.productQuantity}}</text>
									<text class="price">{{orderItem.price / 100.0}}</text>
								</view>
							</view>

							<view class="price-box">
								共
								<text class="num">{{item.orderCount}}</text>
								件商品 实付款
								<text class="price">{{item.payAmount / 100.0}}</text>
							</view>
						</navigator>
	...
	export default {
		...
		methods: {
			...
			loadData(source) {
				...
				let orderList = that.$api.api.request('/order/getOrder', 'GET', {
					id: -1,
					...
					res.data.forEach(item => {
						...
						item.orderItemList.forEach(orderItem => {
							item.orderCount += orderItem.productQuantity
						})
					})
