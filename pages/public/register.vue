				...
				<view class="input-item">
					...
					<view class="verify-code">
						<input type="mobile" value="" placeholder="6位验证码" maxlength="6"
						 data-key="verifyCode" @input="inputChange" style="width: 50%;" />
						<button class="verify-code-button" style="width: 50%;" :disabled="sendDisabled" @click="doGetVerify">{{sendText}}</button>
					</view>
				</view>
				<view class="input-item">
		...
		methods: {
			...
			doGetVerify() {
				...
				if (!that.phone || that.phone.length != 11) {
					that.$api.api.toast('请输入正确手机号！')
					return
				}
				that.$api.api.request('/sso/getAuthCode', 'GET', {
					telephone: that.phone
				}).then(res => {
					that.sendDisabled = true
					that.sendText = res.data
					let sec = 120
			...
			async doRegister() {
				...
				that.$api.api.request('/sso/register','POST', {
					telephone: that.phone,
					password: that.password,
					authCode: that.verifyCode
				}).then(res => {
